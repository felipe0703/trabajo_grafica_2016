...
/*------------------------------create geometry-------------------------------*/
	...
	GLuint pointsQ_vbo;
	glGenBuffers (1, &pointsQ_vbo);
	glBindBuffer (GL_ARRAY_BUFFER, pointsQ_vbo);
	glBufferData (GL_ARRAY_BUFFER, 12 * sizeof (GLfloat), pointsQ, GL_STATIC_DRAW);
	
	GLuint coloursQ_vbo;
	glGenBuffers (1, &coloursQ_vbo);
	glBindBuffer (GL_ARRAY_BUFFER, coloursQ_vbo);
	glBufferData (GL_ARRAY_BUFFER, 12 * sizeof (GLfloat), coloursQ, GL_STATIC_DRAW);
...
/*------------------------------create shaders--------------------------------*/
	...
	parse_file_into_str ("shader.vert", vertex_shader, 1024 * 256);
	parse_file_into_str ("shader.frag", fragment_shader, 1024 * 256);
...
/*--------------------------create camera matrices----------------------------*/
	...
	float cam_pos[] = {0.0f, -1.0f, 2.0f}; // don't start at zero, or we will be too close
...
/*-----------------------------move camera here-------------------------------*/
		...
		// agacharse
		GLint letra_C= glfwGetKey(g_window, GLFW_KEY_C);	
		if(GLFW_PRESS==letra_C)
		{
			cam_pos[1] = -1.5f;
			cam_moved = true;
		}
...
/*		// saltar
		GLint barra_espacio= glfwGetKey(g_window, GLFW_KEY_SPACE);	
		if(GLFW_PRESS==barra_espacio)
		{
			cam_pos[1] = 0.0f;
			cam_moved = true;
		}
		if (GLFW_RELEASE==barra_espacio)
		{
			cam_pos[1]=-1.0f;
			cam_moved = true;
		}
